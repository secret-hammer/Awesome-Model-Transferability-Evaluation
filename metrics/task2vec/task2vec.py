...
import logging
import metrics.task2vec.variational as variational
...
class Task2Vec:
    ...
    def __init__(self, model, skip_layers=0, max_samples=None, classifier_opts=None,
                 method='montecarlo', method_opts=None, loader_opts=None, bernoulli=False):
        
        assert hasattr(model, '_classifier')
    ...
    def montecarlo_fisher(self, dataset: Dataset, epochs: int = 1):
        ...
        self.model._classifier.requires_grad = False
        self.model._classifier.grad = None
        ...
        for k in range(epochs):
            ...
            for i, (data, target) in enumerate(tqdm(data_loader, leave=False, desc="Computing Fisher")):
                ...
                output = F.interpolate(output['out'], size=(320, 480), mode="bilinear", align_corners=False)
    ...
    def _run_epoch(self, data_loader: DataLoader, model, loss_fn,
    ...
    def _cache_features(self, dataset: Dataset, indexes=(-1,), max_samples=None, loader_opts: dict = None):
        ...
        if loader_opts is None:
            loader_opts = {}
        data_loader = DataLoader(dataset, shuffle=False, batch_size=16,
        ...
        hooks = [self.model._classifier.register_forward_pre_hook(_hook)]
        ...
        self.model._classifier.input_features = torch.cat(self.model._classifier.input_features)
        self.model._classifier.targets = torch.cat(targets)
    ...
    def _fit_classifier(self, optimizer='adam', learning_rate=0.0004, weight_decay=0.0001,
        ...
        if not hasattr(self.model._classifier, 'input_features'):
            raise ValueError("You need to run `cache_features` on model before running `fit_classifier`")
        targets = self.model._classifier.targets.to(self.device)
        features = self.model._classifier.input_features.to(self.device)

        dataset = torch.utils.data.TensorDataset(features, targets)
        data_loader = _get_loader(dataset, **self.loader_opts)

        if optimizer == 'adam':
            optimizer = torch.optim.Adam(self.model._classifier.parameters(), lr=learning_rate, weight_decay=weight_decay)
        elif optimizer == 'sgd':
            optimizer = torch.optim.SGD(self.model._classifier.parameters(), lr=learning_rate, weight_decay=weight_decay)
        else:
        ...
        for epoch in tqdm(range(epochs), desc="Fitting classifier", leave=False):
            ...
            for data, target in data_loader:
                optimizer.zero_grad()
                output = self.model._classifier(data)
                output = F.interpolate(output, size=(320, 480), mode="bilinear", align_corners=False)
    ...
    def extract_embedding(self, model):
        ...
        for name, module in model.named_modules():
            # 去除分类器和bn层
            if module == model._classifier or module in model._classifier:
                continue
